						...
						<div class="portlet-body">
							...
							<?php
								if(isset($_POST['submit'])) {
									...
									$order = $_POST['order'];
									$update = "UPDATE menu SET title_bg='".$title_bg."', title_en='".$title_en."', title_de='".$title_de."', link='".$link."', parent_id='".$parent_id."', `order`='".$order."' WHERE id='$get_id'";

									if ($conn->query($update) === TRUE) {
										echo "<div class=\"note note-success\">
											<p>Линкът е запазен успешно. <a href=\"./menu.php\">Към списъка с линкове</a></p>
										 </div>";
									} else {
								...
								$get_id = $_GET['id'];
								$sql = "SELECT * FROM menu WHERE id='$get_id'";
							...
							<div class="form-group">
										...
										<?php
											...
											if ($resultt->num_rows > 0) {
												...
												while($roww = $resultt->fetch_assoc()) {
													if($roww['id'] == $row['parent_id']) {
														$row_selected = " selected";
													} else {
														$row_selected = "";
													}
													echo "<option value='".$roww['id']."'$row_selected>".$roww['title_bg']."</option>";
												}
											}
										?>
									</select>
								...
								<div class="input-icon right input-medium margin-top-10">
									<label>Позиция</label>
									<input type="number" value="<?php echo $row['order']; ?>" name="order" min="1" />
								</div>
